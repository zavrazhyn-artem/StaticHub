...
class RaidScheduleService
{
    ...
    public function executeScheduleGeneration(StaticGroup $static): void
    {
        ...
        // Load all future autogen events in a single query — we'll reconcile them
        // against expected slots in memory. withCount('attendances') piggybacks
        // RSVP counts into the same SELECT so reconcileStale doesn't re-query per row.
        // Manual events (is_auto_generated=false) are never in this set.
        ...
        $expected = $this->calculateEventTimestamps(
            $raidDays,
            $static->raid_start_time,
            $static->raid_end_time,
            $timezone,
            (int) config('raid.schedule_days_ahead', 30)
        );

        foreach ($expected as $slot) {
            $match = $this->claimMatchingEvent($existing, $slot['start'], $timezone);

            if ($match) {
                // Update in-place — preserves id, RSVPs, and Discord message.
                $match->start_time = $slot['start'];
                $match->end_time   = $slot['end'];
                $match->save();
            } else {
                Event::create([
                    'static_id'         => $static->id,
    ...
    /**
     * Find and remove (claim) an autogen event that falls on the same local-TZ date
     * as the target slot. Returns the claimed event, or null if none matched.
     */
    private function claimMatchingEvent(Collection $candidates, Carbon $targetStart, string $timezone): ?Event
    {
        $targetDate = $targetStart->copy()->setTimezone($timezone)->format('Y-m-d');

        foreach ($candidates as $id => $event) {
            $eventDate = $event->start_time->copy()->setTimezone($timezone)->format('Y-m-d');
            if ($eventDate === $targetDate) {
                $candidates->forget($id);
                return $event;
            }
        }
    ...
    /**
     * Dispose of autogen events that no longer correspond to any expected slot.
     * Events with a Discord message or any RSVPs are demoted to manual (flag=false)
     * instead of deleted — so history/activity isn't lost.
     */
    private function reconcileStale(Collection $stale): void
    {
        foreach ($stale as $event) {
            $hasActivity = !empty($event->discord_message_id) || ($event->attendances_count ?? 0) > 0;
