...
class EventBuilder extends Builder
{
    ...
    public function autoGenerated(): self
    {
        return $this->where('is_auto_generated', true);
    }

    public function futureForStatic(int $staticId): self
    {
        return $this->where('static_id', $staticId)
            ->where('start_time', '>', now());
    }
    ...
    /**
     * Delete all future auto-generated events for a static.
     * Used before regeneration so the regen output is always the single source of truth.
     */
    public function deleteFutureAutoGenerated(int $staticId): int
    {
        return $this->futureForStatic($staticId)
            ->autoGenerated()
            ->delete();
    }
