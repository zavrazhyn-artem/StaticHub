...
class EventBuilder extends Builder
{
    ...
    public function deleteFutureAutoGeneratedForDayOfWeek(int $staticId, int $carbonDayOfWeek, string $timezone = 'UTC'): int
    {
        return $this->where('static_id', $staticId)
            ->where('start_time', '>', now())
            ->where('description', 'Auto-generated raid session.')
            ->whereRaw("DAYOFWEEK(CONVERT_TZ(start_time, 'UTC', ?)) = ?", [$timezone, $carbonDayOfWeek])
            ->delete();
    }
}
