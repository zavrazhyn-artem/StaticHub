...
@php
    $sortLink = function (string $column, string $label) use ($filters) {
        ...
        $url = route('admin.user-activity.users', array_filter([
...
@section('content')
    ...
    <div style="margin-bottom: 2rem;">
        <h1 style="font-family: 'Space Grotesk', sans-serif; font-size: 1.5rem; font-weight: 700;">Top users</h1>
        <p style="color: #888; font-size: 0.875rem;">Click a row to drill down into that user's history.</p>
    </div>
    ...
    <div style="display: flex; flex-wrap: wrap; gap: 0.75rem; margin-bottom: 1.5rem; align-items: center;">
        ...
        <form method="GET" action="{{ route('admin.user-activity.users') }}" style="display: flex; gap: 0.5rem; align-items: center; flex: 1; max-width: 500px;">
            <div style="position: relative; flex: 1;">
                ...
                <input type="text" name="q" value="{{ $filters['q'] }}" placeholder="Search by name, battletag or static…"
            ...
            @if($filters['q'] !== '')
                <a href="{{ route('admin.user-activity.users', ['period' => $filters['period'], 'sort' => $filters['sort'], 'dir' => $filters['dir']]) }}" class="admin-btn admin-btn-ghost">Clear</a>
            @endif
        </form>
    </div>
            ...
            <thead>
                <tr>
                    @foreach([
                        ...
                        <th style="{{ in_array($col, ['views', 'last_seen'], true) ? 'text-align: right;' : '' }}">
