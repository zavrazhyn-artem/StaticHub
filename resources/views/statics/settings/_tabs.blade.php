<div class="flex items-center gap-2 border-b border-white/5 mb-8">
    ...
    @can('manage', $static)
    <a href="{{ route('statics.settings.schedule') }}"
    ...
    <a href="{{ route('statics.settings.discord') }}"
    ...
    <a href="{{ route('statics.settings.logs') }}"
    ...
    <a href="{{ route('statics.settings.wishlist-configs') }}"
       class="px-6 py-4 font-headline text-xs font-bold uppercase tracking-widest transition-all relative group {{ request()->routeIs('statics.settings.wishlist-configs') ? 'text-slate-400' : 'text-on-surface-variant hover:text-white' }}">
        <div class="flex items-center gap-2">
            <span class="material-symbols-outlined text-sm">tune</span>
            {{ __('Wishlist Configs') }}
        </div>
        @if(request()->routeIs('statics.settings.wishlist-configs'))
            <div class="absolute bottom-0 left-0 w-full h-0.5 bg-slate-400 shadow-[0_0_10px_rgba(148,163,184,0.5)]"></div>
        @endif
    </a>
    @endcan
