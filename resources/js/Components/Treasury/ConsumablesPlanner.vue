    ...
    <div class="bg-surface-container border border-white/5 rounded-xl overflow-hidden shadow-2xl backdrop-blur-sm">
        <div class="px-6 py-4 border-b border-white/5 bg-surface-container-high flex justify-between items-center">
            ...
            <div v-show="isSaving" class="flex items-center gap-2">
                ...
                <span class="text-4xs text-yellow-500 font-bold uppercase tracking-widest">{{ __('Saving...') }}</span>
            </div>
        </div>
        ...
        <div class="p-4">
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
                <div class="bg-surface-container-lowest/50 p-3 rounded-lg border border-white/5">
                    <div class="text-4xs font-bold uppercase tracking-widest text-on-surface-variant mb-1">{{ __('Raid Days') }}</div>
                    <div class="text-lg font-headline font-black text-white flex items-center gap-2">
                        ...
                        <span>{{ raidDays }}</span>
                        <a v-if="settingsScheduleUrl" :href="settingsScheduleUrl" class="ml-auto text-5xs text-yellow-500 hover:underline uppercase tracking-widest">{{ __('Edit') }}</a>
                    </div>
                </div>
                <div class="bg-surface-container-lowest/50 p-3 rounded-lg border border-white/5">
                    <div class="text-4xs font-bold uppercase tracking-widest text-on-surface-variant mb-1">{{ __('Total Members') }}</div>
                    <div class="text-lg font-headline font-black text-white flex items-center gap-2">
            ...
            <div class="mt-4 pt-4 border-t border-white/5 flex flex-col gap-3">
                <div class="grid grid-cols-3 gap-3">
                    <div class="flex flex-col">
                        <div class="text-4xs font-bold uppercase tracking-widest text-on-surface-variant">{{ __('Weekly Total') }}</div>
                    ...
                    <div class="border-x border-white/5 px-3 flex flex-col">
                        <div class="text-4xs font-bold uppercase tracking-widest text-on-surface-variant">{{ __('Tax/Raider') }}</div>
                    ...
                    <div class="flex flex-col">
                        <div class="text-4xs font-bold uppercase tracking-widest text-on-surface-variant leading-tight">{{ __('AH Cost') }}</div>
                        ...
                        <div class="mt-1.5" v-show="savings > 0">
                            <span class="inline-flex px-1.5 py-0.5 rounded bg-success-neon/10 text-success-neon text-5xs font-bold uppercase tracking-widest">
