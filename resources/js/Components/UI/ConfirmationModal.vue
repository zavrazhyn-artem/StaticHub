    ...
    <GlassModal :show="show" max-width="max-w-sm" z-index="z-[110]" backdrop="bg-black/90" @close="emit('close')">
        <div class="p-6 space-y-6">
            ...
            <div class="flex justify-end gap-3">
                <button @click="emit('close')" class="px-6 py-2.5 rounded-lg text-3xs font-bold uppercase tracking-wider text-on-surface-variant hover:text-white transition-colors">
                    {{ cancelLabel }}
                </button>
                <button @click="emit('confirm')" :disabled="loading" class="px-6 py-2.5 rounded-lg text-3xs font-bold uppercase tracking-wider hover:brightness-110 transition-all text-white disabled:opacity-50" :class="variant === 'danger' ? 'bg-error-dim' : 'bg-amber-500'">
