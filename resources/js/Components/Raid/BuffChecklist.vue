...
<template>
    <div class="bg-surface-container/60 border border-white/5 rounded-xl p-3 backdrop-blur-sm">
        <div v-for="section in coverage" :key="section.label" class="mb-2 last:mb-0">
            <div class="text-[8px] font-black uppercase tracking-[0.15em] text-on-surface-variant/50 mb-1.5">{{ section.label }}</div>
            <div class="flex flex-wrap gap-1">
                <div
                    v-for="item in section.items"
                    :key="item.name"
                    class="flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-bold border transition-colors"
                    :class="item.present
                        ? 'bg-green-500/10 border-green-500/20 text-green-400'
                        : 'bg-red-500/10 border-red-500/20 text-red-400'"
                    :title="item.providers.join(', ')"
                >
                    <span class="material-symbols-outlined text-[10px]">{{ item.present ? 'check' : 'close' }}</span>
                    <span>{{ item.name }}</span>
        ...
        <div v-if="missingItems.length" class="mt-2 pt-2 border-t border-white/5">
            <div class="flex items-center gap-1 text-[9px] text-red-400">
