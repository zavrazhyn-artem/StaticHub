        ...
        <table class="w-full text-left text-[11px] border-collapse">
            ...
            <thead>
                ...
                <tr class="bg-black/20 text-gray-500 text-[8px] uppercase tracking-widest font-bold border-b border-white/5 h-8">
                    <th class="p-2 pl-4 w-[240px] min-w-[240px]">Character</th>
                    <!-- iLvL hidden in raids and gear -->
                    <th v-if="activeTab !== 'raids' && activeTab !== 'gear'" class="p-2 text-center border-l border-white/5 w-[60px]">iLvL</th>

                    <template v-if="activeTab === 'summary'">
                        <th colspan="6" class="p-2 text-center border-l border-white/5">Tier Pieces</th>
                        <th class="p-2 text-center border-l border-white/5">M+ Runs</th>
                        <th class="p-2 text-center border-l border-white/5">M+ Rating</th>
                        <th class="p-2 text-center border-l border-white/5">Audit</th>
                        <th class="p-2 text-center border-l border-white/5 w-[130px]">Access Role</th>
                        <th class="p-2 text-center border-l border-white/5 w-[130px]">Roster Status</th>
                    </template>
                ...
                <tr class="bg-black/40 text-cyan-400 text-[9px] uppercase tracking-widest font-bold border-b border-white/5 h-10">
                    <th class="px-4 py-2 w-[240px] min-w-[240px]">Name</th>
                    <th v-if="activeTab !== 'raids' && activeTab !== 'gear'" class="px-2 py-2 text-center w-[60px]">Avg</th>
                    ...
                    <template v-if="activeTab === 'summary'">
                        ...
                        <th class="p-2 text-center border-l border-white/5">Runs</th>
                        <th class="p-2 text-center border-l border-white/5">Rating</th>
                        <th class="p-2 text-center border-l border-white/5">Issues</th>
                        <th class="p-2 text-center border-l border-white/5 w-[130px]">Role</th>
                        <th class="p-2 text-center border-l border-white/5 w-[130px]">Status</th>
                    </template>
                    ...
                    <tr v-if="members.length > 0 && roleKey !== 'unknown'"
                        ...
                        <td colspan="100%" class="py-1 px-4 text-[10px] font-bold uppercase tracking-widest text-cyan-400">
                            <div class="flex items-center gap-2">
                                ...
                                <span>{{ roles.find(r => r.id === roleKey)?.label || roleKey }} ({{ members.length }})</span>
                            </div>
                        </td>
                    </tr>
