    ...
    <GlassModal :show="show" max-width="max-w-lg" @close="$emit('close')">
        ...
        <div class="px-8 pt-8 pb-4 border-b border-white/5">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <span class="text-3xs font-bold uppercase tracking-widest text-primary">
                        {{ __('Onboarding Help') }}
                    </span>
                </div>
            </div>
        ...
        <div class="px-8 py-6 space-y-6">
            ...
            <div v-if="currentHelp.tips.length" class="space-y-3">
                <h4 class="text-3xs font-bold uppercase tracking-widest text-on-surface-variant/60">
                    {{ __('Tips') }}
                </h4>
                <div
        ...
        <div class="px-8 pb-6">
            <button
                @click="$emit('close')"
                class="w-full bg-white/5 border border-white/10 text-on-surface-variant font-bold text-3xs uppercase tracking-[0.2em] py-3 rounded-lg hover:bg-white/10 hover:text-white transition-all"
            >
                {{ __('Got it') }}
            </button>
        </div>
