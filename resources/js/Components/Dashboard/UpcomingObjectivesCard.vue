                ...
                >
                    <div class="flex items-center gap-2.5">
                        <div class="w-8 h-8 rounded-md bg-surface-container-highest flex flex-col items-center justify-center border border-white/5 group-hover:border-primary/50 transition-colors">
                            <span class="text-[7px] font-bold text-on-surface-variant uppercase tracking-tighter leading-none">{{ event.month }}</span>
                            <span class="text-sm font-black text-white leading-none">{{ event.day }}</span>
                        </div>
                        <div>
                            <div class="text-[9px] font-bold text-white uppercase tracking-wider">{{ event.dayOfWeek }}</div>
                            <div class="text-[9px] text-on-surface-variant flex items-center gap-1">
                                <span class="material-symbols-outlined text-[9px]">schedule</span>
                                {{ event.time }}
                                <span class="opacity-20">|</span>
                                <span class="flex items-center gap-0.5">
                                    <span class="material-symbols-outlined text-[9px] text-success-neon">groups</span>
                                    {{ event.rsvpCount }}
                                </span>
                            </div>
                        </div>
                    </div>
                    <span class="bg-surface-container-highest group-hover:bg-primary/20 group-hover:text-primary text-on-surface-variant px-2 py-0.5 rounded text-[7px] font-black uppercase tracking-widest transition-all">{{ __('INTEL') }}</span>
                </a>
            </template>
            <div v-else class="text-center py-3 text-on-surface-variant italic text-[10px] border border-dashed border-white/5 rounded-lg">
